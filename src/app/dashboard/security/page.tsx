...
export default function SecurityPage() {
  ...
  const inputCls = "w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-500/20 transition-all pr-10";
    ...
    <div className="min-h-dvh bg-slate-50">
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Manage your password, 2FA, and active sessions.</p>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400">Use a strong, unique password.</p>
            </div>
          </div>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-slate-400">Add an extra layer of security with an authenticator app.</p>
            </div>
            {/* Toggle */}
            <button
              onClick={() => { setTwoFaEnabled(e => !e); setShowQr(false); }}
              className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${twoFaEnabled ? "bg-indigo-500" : "bg-slate-200"}`}
            >
          ...
          <div className="px-5 py-4">
            {!twoFaEnabled ? (
              <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50 border border-amber-200">
                ...
                <div>
                  <p className="text-xs font-semibold text-amber-700">2FA is disabled</p>
                  <p className="text-[11px] text-amber-600 mt-0.5">Enable two-factor authentication to better protect your account.</p>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-xl bg-green-50 border border-green-200">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-green-700">2FA is enabled</p>
                    <p className="text-[11px] text-green-600 mt-0.5">Your account is protected with an authenticator app.</p>
                  </div>
                </div>
                <button
                ...
                {showQr && (
                  <div className="flex flex-col items-center gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200">
                    {/* Fake QR code pattern */}
                    <div className="w-28 h-28 bg-white border border-slate-200 rounded-lg grid grid-cols-7 gap-px p-1.5">
                      {Array.from({ length: 49 }).map((_, i) => (
                        <div key={i} className={`rounded-sm ${[0,1,2,3,4,5,7,12,14,19,21,22,23,24,25,26,28,33,35,40,42,43,44,45,46,47,48].includes(i) ? "bg-slate-800" : "bg-white"}`} />
                      ))}
                    </div>
                    <p className="text-[11px] text-slate-400 text-center">Scan with Google Authenticator or Authy</p>
                    <code className="text-[10px] text-slate-500 bg-slate-100 px-2 py-1 rounded font-mono tracking-wider">JBSWY3DPEHPK3PXP</code>
                  </div>
                )}
              </div>
            )}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-100">
            ...
            <div className="flex-1">
              ...
              <p className="text-[11px] text-slate-400">{sessions.length} device{sessions.length !== 1 ? "s" : ""} signed in</p>
            </div>
          </div>
                ...
                <div key={s.id} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {s.current && (
                        <span className="px-1.5 py-0.5 rounded-full bg-green-50 border border-green-200 text-green-600 text-[9px] font-bold">CURRENT</span>
                      )}
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5 truncate">{s.location} · {s.ip}</p>
                  ...
                  {!s.current && (
                    <button
                      ...
                      className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-red-200 bg-red-50 text-red-500 text-[11px] font-semibold hover:bg-red-100 transition-colors disabled:opacity-50"
                    >
                      <LogOut size={10} />
